...
@pytest.fixture
def client():
    cfg = Config(agent_token="A", app_token="P", db_path=":memory:",
                 apns_key_path=None, apns_key_id=None, apns_team_id=None,
                 apns_bundle_id="com.holdmyagent.HoldMyAgent", apns_sandbox=True)
...
def test_create_requires_agent_token(client):
    assert _create(client, tok="P").status_code == 403
...
def test_list_and_decide(client):
    ...
    lst=client.get("/v1/requests?status=pending", headers={"Authorization":"Bearer P"})
    assert lst.status_code==200 and len(lst.json())==1
    d=client.post(f"/v1/requests/{rid}/decision", headers={"Authorization":"Bearer P"},
                  json={"decision":"approve"})
    assert d.status_code==200 and d.json()["status"]=="approved"
    d2=client.post(f"/v1/requests/{rid}/decision", headers={"Authorization":"Bearer P"},
                   json={"decision":"deny"})
    assert d2.status_code==409

def test_unknown_404(client):
    assert client.get("/v1/requests/nope", headers={"Authorization":"Bearer A"}).status_code==404

def test_device_register(client):
    r = client.post("/v1/devices", headers={"Authorization": "Bearer P"},
...
def test_pair_page_returns_html(client):
    r = client.get("/pair?token=P")  # 'P' is the app token in the fixture
    assert r.status_code == 200
...
def test_pair_page_contains_payload(client):
    ...
    r = client.get("/pair?token=P")
    assert "holdmyagent://pair" in r.text
    assert "token=P" in r.text
