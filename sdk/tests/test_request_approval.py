...
def _transport(status_after=1, decision="approved", capture=None):
    ...
    def handler(request):
        if capture is not None and request.method == "POST":
            import json; capture.update(json.loads(request.read()))
...
def test_approved_and_fields_sent(monkeypatch):
    monkeypatch.setenv("HMA_SERVER_URL", "http://test"); monkeypatch.setenv("HMA_AGENT_TOKEN", "t")
    sent = {}
...
def test_server_down_is_denied(monkeypatch):
    monkeypatch.setenv("HMA_SERVER_URL", "http://127.0.0.1:1"); monkeypatch.setenv("HMA_AGENT_TOKEN", "t")
